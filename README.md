...
旧仓库的笔记连同 `附件/` 目录**整体拖入 `待处理笔记/` 即可**，无需拆散：守护进程递归监听子目录、递归按文件名定位附件——wikilink 无论带不带路径（`![[xx.png]]` 或 `![[附件/xx.png]]`）、标准链接 `[x](附件/a.pdf)`、HTML 内嵌标签 `<img src="附件/xx.png">`（Kindle/HTML 转 Markdown 产物常用，v1.6.3 起支持）都能解析。**附件不限可解析类型（v1.6.4 起）**：笔记中链接的 `.zip`/`.epub`/`.dwg` 等任意格式文件同样随迁到目标目录的 `附件/` 子目录，不可解析的类型不在文末转录、仅如实标注占位；归档后正文 wikilink 原位保留，**未被任何笔记链接的孤立文件保留在收件箱**待人工处置。
...
**无需手动建目录**：目录树中没有合适目录时，LLM 会依据笔记主题自动新建简洁的一级目录（如「开发环境」「AI 工具」「网络工具」），同主题笔记后续自动复用同一目录——分类体系随归档自然生长。提示词明令禁止输出「未分类」「笔记」「文档」「其他」等无信息量目录名（旧版本空仓库冷启动时首篇易回退「未分类」、后续笔记持续跟随的雪球效应已根治）。链接收藏、账号信息等超短碎片笔记也会按实际用途归类（提示词明确禁止凭个别词语的弱关联塞入已有目录）。

想进一步约束分类风格，可编辑 `ai_context.md` 的「AI 处理规则」区写明约定（该区域每次注入给模型），例如：「开发工具与运行环境类归入『开发环境』；大模型与智能体应用归入『AI 工具』」。**存量笔记重新归类 / 误归档纠正**：整篇（连同其附件）移回收件箱即可自动重新归档——正文原样保留（原文保留模式），目录/文件名/标签等元数据重新生成；`ai_context.md` 中的同名旧条目会被自动替换（不会被旧归档记录锚定），指向旧路径的索引死链也会在增量同步中被自动剔除。
