...
2. 归档完成后收件箱内残留的空 `附件/` 目录不会被自动清理，手动删除即可
3. 建议小批量分批投入（每批观察归类结果），一次性倒入大量草稿会排队逐篇处理（每篇 10~60s）

### 归类全是「未分类」怎么办（冷启动雪球）

空仓库冷启动时 LLM 没有目录树可参照，首篇极易回退到「未分类」（`fallback_folder` 默认值）；而归档规则要求「**优先从目录树中选择已有目录**」——一旦「未分类」目录出现，后续笔记就会持续跟随，越滚越多。破解三步：

1. **建目录**：在 Obsidian 中按自己的体系建好一级目录（如「开发环境」「AI 工具」「网络工具」）——目录树每次都会注入给模型，新笔记自动归位
2. **写规则**：编辑 `ai_context.md` 的「AI 处理规则」区（把示例文字替换为真实约定，该区域每次注入给模型），例如：「Conda/Docker/Git/Python 等开发工具类归入『开发环境』；Ollama/大模型/智能体应用归入『AI 工具』；GitHub/镜像/代理类归入『网络工具』」
3. **存量归位**：已落入「未分类」的笔记直接在 Obsidian 中拖到目标目录（文件名与 frontmatter 保留，RAG 检测到 mtime 变化自动重新索引）；顺手更新或删除 `ai_context.md`「历史归档索引」里的 `[[未分类/…]]` 旧条目，避免旧路径持续误导后续归档（该文件改动同样会被自动重索引）
