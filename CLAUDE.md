...
## 铁律：真实测试样本内容严禁入库（用户 2026-09-01 要求，永久有效）

用户提供的真实手写测试样本（PDF/图片/标注文件）中的一切文字——正文引句、
人名、地名、机构名、专有名词、OCR 误差示例（错字对照）——严禁写入任何
入库内容：README / PROJECT_DOC / 代码注释 / docstring / 提交信息 / 测试
用例。文档与测试一律用合成样本或「真实样本 A/B」这类去内容化指代；提交
前用 `git grep` 对样本关键词做最后复查。`tmp/` 已加入 .gitignore，样本
文件永不入库。
