...
| 版本 | 日期 | 变更摘要 | git tag |
|---|---|---|---|
| 1.9.0 | 2026-09-01 | 功能（用户需求：① 图像附件的手写文字能否识别；② 按作者手工写在每篇草稿正文最前面的要求归档）：① **图像 OCR 双引擎**——新增 `ocr.image_engine`（`auto` 默认 / `vision` / `rapidocr` / `off`，CONFIG_DEFAULTS + config.json + config.example.json 三处同步）：auto = Vision（ocrmac，Neural Engine 零延迟，印刷体与规整手写佳）先识别，无结果或异常时 RapidOCR（PP-OCRv6，中文手写友好）兜底，两引擎均空（纯图形）如实占位；实现上 `ocr_image` 拆出 `_vision_image`（原始文本层）+ 新增 `_rapidocr_image` / `extract_image` 引擎路由，`dispatch_attachment` 图像分支改调 `extract_image`，kind 动态标签（「图像 OCR（Vision）」/「（RapidOCR）」/「（Vision→RapidOCR 兜底）」/「（已关闭）」）；任何失败降级占位不中断归档；HEIC 仅 Vision 支持（OpenCV 不读 HEIC）；② **草稿指令块**——正文最前（frontmatter 之后、任何正文内容之前）的 ```smartvault 围栏块 = 作者对本篇的归档要求：新增 `parse_draft_instructions`（纯正则确定性提取，中段同名块是普通内容不动、空块/无块零副作用、剥离由代码完成非 LLM 决定，原稿备份 .smartvault/backup/）与 `parse_rewrite_directive`（「整理正文：是/否」为**唯一确定性覆盖项**，仅本篇覆盖全局 content_rewrite 且仍受 rewrite_max_chars 上限约束——正文改写是 v1.3.1 事故红线，自然语言请求不生效）；指令全文注入 `build_user_prompt` 新增高优先级段（优先级：输出契约 > 作者要求 > ai_context 规则与一般整理规则），SYSTEM_PROMPT 新增规则 9（作者指定目录可新建，仍受深度与禁用名约束）；`run_pipeline` 读取草稿后即提取（后续附件引用扫描、正文长度判定均基于剥离后正文）。新增 23 项单测（tests/test_image_ocr.py 9 项：mock `_vision_image` + `_RAPIDOCR_STATE` 注入 fake 引擎，验证三模式分发/auto 空结果兜底/Vision 异常救场/引擎不可用占位/off/dispatch 透传；tests/test_draft_instructions.py 14 项：位置判定/frontmatter 兼容/大小写/空块/整理正文开关变体/Prompt 注入位置与 SYSTEM_PROMPT 锁定回归；共 132 项全绿）。函数级实测：逐字扰动模拟潦草手写（±8° 旋转 + ±5px 抖动 + 灰度/字号随机）28 字 RapidOCR 28/28 全对、Vision 26/28（1 错字）；E2E 两篇 `--once` 退出码均 0：A=指令块 + 潦草手写 PNG 附件（归入指定目录「E2E测试」/指定文件名/指定标签全命中、正文逐字保留 + 双链 5 处、转录折叠块「图像 OCR（Vision）」如实含 Vision 误差、附件落位 `附件/`）；B=整理正文：是（覆盖开关生效日志、正文被列表化整理且事实保留、目录按要求）；测试产物已清理；README + 本文档模块地图/配置表/流程时序/排障表同步 | `v1.9.0` |
...
| 多模态解析 | `dispatch_attachment`（按扩展名分发） | png/jpg…→`extract_image` 引擎路由（v1.9.0 `ocr.image_engine`：auto 默认=Vision 先识别、失灵时 RapidOCR 兜底；vision/rapidocr 单引擎；HEIC 仅 Vision）；音视频→mlx-whisper（自动回退 openai-whisper）；pdf→PyMuPDF 文本层优先、扫描页（中文手写）RapidOCR（PP-OCRv6）兜底；docx/xlsx/pptx→对应库；pages/numbers/key→包内 QuickLook/Preview.pdf |
| 草稿指令块 | `parse_draft_instructions` `parse_rewrite_directive` | 正文最前（frontmatter 后）的 ```smartvault 围栏块 = 作者归档要求：确定性正则提取（中段同名块不动、空块/无块零副作用）注入 Prompt 高优先级段（输出契约 > 作者要求 > ai_context/一般规则）；「整理正文：是/否」唯一确定性开关，仅本篇覆盖 content_rewrite 且仍受 rewrite_max_chars 约束；指令块剥离不进终稿，原稿备份 .smartvault/backup/ |
...
| `ocr.image_engine` | `auto` | 图像附件 OCR 引擎（v1.9.0）：`auto`（Vision 先识别，无结果/异常时 RapidOCR 兜底）/ `vision` / `rapidocr`（手写图片优先，与 PDF 扫描页同引擎）/ `off`；HEIC 仅 Vision 支持 |
...
1. 读草稿（剥离已有 frontmatter）→ **`parse_draft_instructions` 提取正文最前的 ```smartvault 指令块**（v1.9.0：作者要求注入 Prompt、确定性剥离不进终稿；中段同名块是普通内容）→ `find_attachment_refs` 提取引用 → `resolve_attachment` 在收件箱定位（含无扩展名补全）
2. 等待全部引用附件就位（`attachment_wait_timeout` 兜底放行）
3. 逐个 `dispatch_attachment` 按类型提取文本（图像经 `extract_image` 引擎路由；超过 `attachment_max_chars` 截断）
4. 组装 Prompt：SYSTEM_PROMPT + 目录树 + ai_context（截断后）+ **作者特别要求（如有，v1.9.0 高优先级段）** + 草稿（已剥离指令块）+ 附件文本；「整理正文：是/否」指令仅本篇覆盖 content_rewrite（仍受 rewrite_max_chars 约束）
...
| 手写图像（png/jpg）识别不准/错字 | auto 默认 Vision 优先，Vision 对潦草手写偶有错字（实测扰动模拟 28 字错 1 字，RapidOCR 全对） | `ocr.image_engine: "rapidocr"` 改用 PP-OCRv6（与 PDF 扫描页同引擎）后重启守护进程；HEIC 图片仅 Vision 支持 |
| 草稿指令块未生效 | 块不在正文最前（须在 frontmatter 之后、任何正文内容之前），或围栏词拼写错误 | 移到正文开头并确认为 ```smartvault 围栏；正文中段的同名块按普通内容处理（防误删）；「整理正文」须写成结构化指令「整理正文：是」，自然语言请求不生效 |
